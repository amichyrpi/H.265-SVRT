...
static int decode(svrt_context *c,AVRational time_base,uint64_t packet_pts_us){int rc=avcodec_send_packet(c->decoder,c->packet);if(rc<0&&rc!=AVERROR(EAGAIN)){set_error(c,"avcodec_send_packet failed: %d",rc);return -1;}while((rc=avcodec_receive_frame(c->decoder,c->frame))>=0){uint64_t decoded=atomic_fetch_add(&c->stats.decoded_frames,1)+1;int shown;if(c->cfg.headless)shown=c->cfg.require_zero_copy&&c->frame->format!=AV_PIX_FMT_DRM_PRIME?-1:0;else if(c->frame->format==AV_PIX_FMT_DRM_PRIME&&c->drm)shown=svrt_drm_present(c->drm,c->frame,c->error,sizeof(c->error));else if(!c->cfg.require_zero_copy&&c->renderer)shown=software_present(c,c->frame);else shown=-1;if(shown&&!c->error[0])set_error(c,"decoder returned %s instead of DRM PRIME",av_get_pix_fmt_name(c->frame->format));if(shown)atomic_fetch_add(&c->stats.dropped_frames,1);else{atomic_fetch_add(&c->stats.presented_frames,1);int64_t pts=c->frame->best_effort_timestamp;uint64_t pts_us=pts==AV_NOPTS_VALUE?packet_pts_us:(uint64_t)av_rescale_q(pts,time_base,AV_TIME_BASE_Q);packet_event(c,SVRT_PACKET_PROCESSED,pts_us);}if(decoded==1||decoded%60==0)fprintf(stderr,"SVRT: frame=%llu format=%s shown=%llu dropped=%llu\n",(unsigned long long)decoded,av_get_pix_fmt_name(c->frame->format),(unsigned long long)atomic_load(&c->stats.presented_frames),(unsigned long long)atomic_load(&c->stats.dropped_frames));av_frame_unref(c->frame);if(shown)return -1;}return rc==AVERROR(EAGAIN)||rc==AVERROR_EOF?0:-1;}
int svrt_run(svrt_context *c){if(!c)return -1;svrt_pipe_config pc={.bind_address=c->cfg.bind_address,.port=c->cfg.port,.interrupt=interrupted,.opaque=c};fprintf(stderr,"SVRT: listening for MPEG-TS on TCP %u\n",c->cfg.port);if(svrt_pipe_listen(&c->pipe,&pc,c->error,sizeof(c->error)))return -1;if(open_video(c,svrt_pipe_video_parameters(c->pipe)))return -1;AVRational time_base=svrt_pipe_time_base(c->pipe);c->decoder->pkt_timebase=time_base;int rc=0;while(!atomic_load(&c->stopping)){SDL_Event e;while(!c->cfg.headless&&SDL_PollEvent(&e))if(e.type==SDL_QUIT||e.type==SDL_KEYDOWN)atomic_store(&c->stopping,1);rc=svrt_pipe_read(c->pipe,c->packet);if(rc<0)break;atomic_fetch_add(&c->stats.access_units,1);atomic_fetch_add(&c->stats.bytes_received,(uint64_t)c->packet->size);uint64_t pts_us=c->packet->pts==AV_NOPTS_VALUE?UINT64_MAX:(uint64_t)av_rescale_q(c->packet->pts,time_base,AV_TIME_BASE_Q);if(pts_us!=UINT64_MAX){atomic_store(&c->stats.last_pts_us,pts_us);packet_event(c,SVRT_PACKET_RECEIVED,pts_us);}if(decode(c,time_base,pts_us)){av_packet_unref(c->packet);return -1;}av_packet_unref(c->packet);}if(rc<0&&!atomic_load(&c->stopping)&&!atomic_load(&c->stats.decoded_frames))set_error(c,"video stream ended: %d",rc);return c->error[0]?-1:0;}
