...
static int open_video(svrt_context *c,const AVCodecParameters *parameters){
    ...
    AVDictionary *opts=NULL;av_dict_set(&opts,"num_capture_buffers","12",0);int rc=avcodec_open2(c->decoder,codec,&opts);av_dict_free(&opts);if(rc<0){char e[128];av_strerror(rc,e,sizeof(e));set_error(c,"opening decoder %s failed: %s",codec->name,e);return -1;}
