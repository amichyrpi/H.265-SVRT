...
int svrt_run(svrt_context *c){
    ...
    atomic_store(&c->stopping,1);pthread_mutex_lock(&c->extra_mutex);pthread_cond_broadcast(&c->extra_cond);pthread_mutex_unlock(&c->extra_mutex);pthread_mutex_lock(&c->present_mutex);pthread_cond_broadcast(&c->present_cond);pthread_mutex_unlock(&c->present_mutex);
    if(atomic_load(&c->extra_started))pthread_join(c->extra_thread,NULL);
    if(atomic_load(&c->present_started))pthread_join(c->present_thread,NULL);
