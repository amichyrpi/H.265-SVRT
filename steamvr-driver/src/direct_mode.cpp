...
void SvrtDirectMode::Stop(){
  ...
  CloseEncoder();
  std::lock_guard<std::mutex> l(mutex_);textures_.clear();slots_.clear();context_.Reset();device_.Reset();
}
...
void SvrtDirectMode::PresentVirtual(vr::SharedTextureHandle_t handle){
  if(!device_||!handle)return;
  ComPtr<ID3D11Texture2D> source;
  if(FAILED(device_->OpenSharedResource(reinterpret_cast<HANDLE>(handle),IID_PPV_ARGS(&source)))){debug("could not open virtual-display backbuffer");return;}
  D3D11_TEXTURE2D_DESC d{};source->GetDesc(&d);
  if(d.SampleDesc.Count!=1){debug("virtual-display backbuffer is multisampled");return;}
  std::unique_lock<std::mutex> l(mutex_);std::lock_guard<std::mutex> dl(d3d_mutex_);
  if(!EnsureVirtualSlots(d.Width,d.Height,d.Format))return;
  auto slot=std::find_if(slots_.begin(),slots_.end(),[](const Slot&s){return !s.pending;});if(slot==slots_.end())return;
  context_->CopyResource(slot->staging.Get(),source.Get());context_->Flush();slot->pending=true;slot->sequence=++sequence_;l.unlock();ready_.notify_one();
}
...
void SvrtDirectMode::EncoderThread(){while(running_){std::unique_lock<std::mutex> l(mutex_);ready_.wait(l,[this]{return !running_||std::any_of(slots_.begin(),slots_.end(),[](const Slot&s){return s.pending;});});if(!running_)break;auto it=std::find_if(slots_.begin(),slots_.end(),[](const Slot&s){return s.pending;});if(it==slots_.end())continue;Slot *slot=&*it;unsigned w=width_,h=height_;uint64_t frame=slot->sequence;l.unlock();D3D11_MAPPED_SUBRESOURCE map{};bool ok=false;
  ...
  // wait behind the encoder worker and produced visible compositor hitches.
  {
    ...
    ok=!encoder_failed_&&SUCCEEDED(context_->Map(slot->staging.Get(),0,D3D11_MAP_READ,0,&map));
  }
  if(ok){
    frame_.resize((size_t)w*h*4);
  ...
  l.lock();slot->pending=false;if(ok&&(frame==1||frame%fps_==0))debugf("encoded frame=%llu size=%ux%u",(unsigned long long)frame,w,h);if(!ok&&running_){encoder_failed_=true;debug("encoder pipe write failed");}}}
