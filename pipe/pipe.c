...
int svrt_pipe_make_url(char *out,unsigned n,const svrt_pipe_config *c){if(!out||!n||!c)return -1;int used=snprintf(out,n,"tcp://%s:%u?listen=1&tcp_nodelay=1&recv_buffer_size=65536",c->bind_address?c->bind_address:"0.0.0.0",c->port?c->port:9944);return used<0||(unsigned)used>=n?-1:0;}
